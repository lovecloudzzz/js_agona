...
export function filterPersons<T extends Person>(persons: Person[], personType: 'user' | 'admin', criteria: Criteria<T>): T[] {
    return persons
        .filter((person): person is T => person.type === personType)
        .filter((person) => {
            let criteriaKeys = Object.keys(criteria) as (keyof Criteria<T>)[];
